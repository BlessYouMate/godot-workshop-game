...
func interact(user: Node2D):
	if (player_data.inventory.hands_string == "empty"):
		print("pusta reka")
		if(get_parent().items_on_shelf[1]["is_occupied"] == 1):
			print("zajete")
			# aktualizacja gracza
			player_data.inventory.hands_string = get_parent().items_on_shelf[1]["hand_desc"]
			player_data.player.hand_tscn = get_parent().items_on_shelf[1]["scene"]
			# aktualizacja szafki
			get_parent().items_on_shelf[1]["is_occupied"] = 0
			get_parent().items_on_shelf[1]["hand_desc"] = "empty"
			get_parent().items_on_shelf[1]["scene"] = PackedScene
		...
		print("pelna reka")
		if(player_data.inventory.hands_string == get_parent().items_on_shelf[1]["hand_desc"]):
			print("taki sam item")
			return
		else:
			print("inny item")
			...
			if(get_parent().items_on_shelf[1]["is_occupied"] == 1):
				get_child(1).queue_free()
			get_parent().items_on_shelf[1]["hand_desc"] = hands_string_tmp
			get_parent().items_on_shelf[1]["scene"] = hands_tscn_tmp
			var item = hands_tscn_tmp.instantiate()
			self.add_child(item)
			item.position.x = 0
			item.position.y = -4
			get_parent().items_on_shelf[1]["is_occupied"] = 1
			update_hand()
			return
